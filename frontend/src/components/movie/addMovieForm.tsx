...
import { createMovieApi } from "@/api/movie/movieApi";
...
function AddMovieForm({ postSubmit }: { postSubmit?: () => void }) {
  ...
  const addMovieAction = async (
    ...
    const result = await createMovieApi({
      name,
  ...
  const [state, formAction, pending] = useActionState(addMovieAction, {
    errors: initialFormState,
  });
